...
@interface VScroll2Page () <UIScrollViewDelegate> {
...
- (void)selectNameButton:(UIButton *)sender {
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(scrollPage:didSelectPageAtIndex:)]) {
                [self.delegate scrollPage:self didSelectPageAtIndex:_curPage];
            }
        }];
    }
    //重复点击选中按钮

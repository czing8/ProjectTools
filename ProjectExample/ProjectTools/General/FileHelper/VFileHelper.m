...
@interface VFileHelper()

@property (strong, nonatomic) NSFileManager *manager;

@end

@implementation VFileHelper
...
+ (NSArray *)listFilesInTmpDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[self tmpDirectory] deep:deep];
}
...
+ (BOOL)clearTmpDirectory {
    ...
    for (NSString *file in subFiles) {
        NSString *absolutePath = [[self tmpDirectory] stringByAppendingPathComponent:file];
        isSuccess &= [self removeItemAtPath:absolutePath];
    }
    return isSuccess;
}
